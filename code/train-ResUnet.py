...
saved_model_path = "resunet_dice_E10_back.pth.tar"
...
if load_model:
    load_checkpoint(torch.load("resunet_dice_E5_back.pth.tar"))
...
#         model.eval()
    ...
    for batch_idx, (image, mask) in tqdm(enumerate(train_loader)):
        ...
        for i in range(6,150,patch_size):
            no_of_patch = no_of_patch + 1
            image_new = image[:,:,:,:,i:i+patch_size]
            ...
            dice_coeff,dice_coeff0,dice_coeff1,dice_coeff2,dice_coeff3 = multi_class_dice(mask_new.cuda(),predicted_mask.cuda())
            

            print("Total Dice loss "+str(dice_coeff))
    ...
    with torch.no_grad():
        for batch_idx, (image, mask) in tqdm(enumerate(loader)):
            ...
            for i in range(6,150,patch_size):
                ...
                pred = predicted_mask.squeeze(0).permute(3,2,1,0)
                target = mask_new.squeeze(0).permute(3,2,1,0)
            ...
            final_predicted[:,:,:,:,150:] = mask[:,:,:,:,150:]
            
            dice_coeff,dice_coeff0,dice_coeff1,dice_coeff2,dice_coeff3 = multi_class_dice(mask.cuda(),final_predicted.cuda())
            
            
            print("Dice loss for background image "+str(dice_coeff0))
            print("Dice loss for tumor 1 image "+str(dice_coeff1))
            print("Dice loss for tumor 2 image "+str(dice_coeff2))
            print("Dice loss for tumor 3 image "+str(dice_coeff3))
            print("Total Dice loss "+str(dice_coeff))
            
            total_dice+=dice_coeff
            total_dice0+=dice_coeff0
            total_dice1+=dice_coeff1
            total_dice2+=dice_coeff2
            total_dice3+=dice_coeff3
            
            print("Bce Loss at batch "+str(batch_idx)+" = "+str(batch_loss.item()/no_of_patch))
            
        print("Total binary cross entropy loss =  "+str(total_loss.item()/no_of_batch))
        print("Final Dice loss for background image "+str(total_dice0/no_of_batch))
        print("Final Dice loss for tumor 1 image "+str(total_dice1/no_of_batch))
        print("Final Dice loss for tumor 2 image "+str(total_dice2/no_of_batch))
        print("Final Dice loss for tumor 3 image "+str(total_dice3/no_of_batch))
        print("Final Total Dice loss "+str(total_dice/no_of_batch))
