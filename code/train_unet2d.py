...
saved_model_path = "unet_2D_E5_metric.tar"
...
#         model.eval()
    ...
    for batch_idx, (image, mask) in tqdm(enumerate(train_loader)):
        ...
        for i in range(6,105,patch_size):
            ...
            image_new = image_new.squeeze(0).permute(3,0,1,2)
            ...
            dice_coeff,dice_coeff0,dice_coeff1,dice_coeff2,dice_coeff3 = multi_class_dice2(mask_new.cuda(),predicted_mask.cuda())
            
            
            print("Total Dice coeff "+str(dice_coeff)+" at epoch "+str(epoch))
            print("Dice loss for background image "+str(dice_coeff0))
            print("Dice coeff for tumor 1 "+str(dice_coeff1))
            print("Dice coeff for tumor 2 "+str(dice_coeff2))
            print("Dice coeff for tumor 3 "+str(dice_coeff3))
...
#             final_predicted[:,:,:,:,134:] = mask[:,:,:,:,134:]
        ...
        print("Final Dice loss for tumor 1 image "+str(total_dice1/(no_of_batch*4)))
        print("Final Dice loss for tumor 2 image "+str(total_dice2/(no_of_batch*4)))
        print("Final Dice loss for tumor 3 image "+str(total_dice3/(no_of_batch*4)))
        print("Final Total Dice loss "+str(total_dice/(no_of_batch*4)))
